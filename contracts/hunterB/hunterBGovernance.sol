...
contract HunterBGovernance is Initializable,UUPSUpgradeable,GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    string public constant name = "HunterB Governor Bravo";
    ...
    /// @notice hunter price to create proposal
    uint public hunterPriceProposal;

    /**
      * @notice Used to initialize the contract during delegator constructor
      * @param timelock_ The address of the Timelock
      * @param hunterB_ The address of the hunterB token
      * @param votingPeriod_ The initial voting period
      */
    function initialize(address timelock_, address hunterB_, uint votingPeriod_, address treasury_, uint proposalPeriod_, uint hunterPriceProposal_) public initializer{
        require(address(timelock) == address(0), "GovernorBravo::initialize: can only initialize once");
        require(timelock_ != address(0), "GovernorBravo::initialize: invalid timelock address");
        require(hunterB_ != address(0), "GovernorBravo::initialize: invalid hunterB address");
        require(treasury_ != address(0), "GovernorBravo::initialize: invalid treasury address");

        timelock = TimelockInterface(timelock_);
        hunterB = hunterBInterface(hunterB_);
        votingPeriod = votingPeriod_;
        admin = timelock_;
        treasury = treasury_;
        governanceStartTime = block.timestamp;
        proposalPeriod = proposalPeriod_;
        hunterPriceProposal = hunterPriceProposal_;
    }
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(hunterB.iHunterMandator(msg.sender) != true,"GovernorBravo::propose: Non Hunter Manders cannot create proposal");
        require(TreasuryInterface(treasury).hunterPriceInUSD(hunterB.balanceOf(msg.sender)) >= hunterPriceProposal,"GovernorBravo::propose: only hunterB holders with enough balance");
    ...
    function castVoteInternal(address voter, uint proposalId, uint8 support) internal returns (uint256) {
        require(hunterB.iHunterMandator(voter),"GovernorBravo::castVoteInternal: Non Hunter Manders cannot vote");
