...
contract HunterA is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    // Bonus muliplier for early Hunter makers.
    ...
    // total Hunter used for purchase land
    uint256 public totalHunterUsedForPurchase;
    ...
    function initialize(        
        ...
        __ERC20_init_unchained("hunterA", "hunterA");
        __Pausable_init_unchained();
        ERC20PermitUpgradeable.__ERC20Permit_init("hunterA");
    ...
    function setTopStakerNumber(uint256 _topStakerNumber) public onlyOwner {
        topStakerNumber = _topStakerNumber;
    }
    ...
    function pendingHunter(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalHunterStaked.sub(totalHunterUsedForPurchase));
    ...
    /**
    @notice Sorting the highest Hunter staker in pool
    @param _pid : pool id
    @param left : left
    @param right : right
    @dev Description :
        It is used for sorting the highest Hunter staker in pool. This function definition is marked
    ...
    */
    ...
    @dev Description :
    DAO governance will be performed by the top 50 wallets with the highest amount of staked Hunter tokens.
    */
    ...
    /**
    @notice Hunter staking track the Highest 50 staked users
    @param _pid : pool id
    ...
    */
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            pool.lastHunterRewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalHunterStaked.sub(totalHunterUsedForPurchase));
        }
    ...
    function claimHunter(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
    ...
    // Safe Hunter transfer function to admin.
    function accessHunterTokens(uint256 _pid, address _to, uint256 _amount) public {
        require(msg.sender == adminAddress, "sender must be admin address");
        require(totalHunterStaked.sub(totalHunterUsedForPurchase) >= _amount, "Amount must be less than staked Hunter amount");
        PoolInfo storage pool = poolInfo[_pid];
        uint256 HunterBal = pool.lpToken.balanceOf(address(this));
        if (_amount > HunterBal) {
            pool.lpToken.transfer(_to, HunterBal);
            totalHunterUsedForPurchase = totalHunterUsedForPurchase.add(HunterBal);
            emit EmergencyWithdraw(_to, _pid, HunterBal);
        } else {
            pool.lpToken.transfer(_to, _amount);
            totalHunterUsedForPurchase = totalHunterUsedForPurchase.add(_amount);
